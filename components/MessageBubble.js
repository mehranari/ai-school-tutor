                    ...
                    >
                        <div className={`prose prose-sm sm:prose-base max-w-none math-content overflow-hidden ${isAi ? "text-slate-700" : "text-white prose-invert"
                            ...
                            <ReactMarkdown
                                ...
                                components={{
                                    h1: ({ node, ...props }) => <h1 className={`text-xl sm:text-3xl font-black mb-4 sm:mb-8 mt-2 sm:mt-4 text-slate-900 border-l-4 border-${accent}-500 pl-4 sm:pl-6 break-words`} {...props} />,
                                    h2: ({ node, ...props }) => <h2 className="text-lg sm:text-2xl font-bold mb-4 sm:mb-6 mt-6 sm:mt-10 pb-2 border-b border-slate-100 text-slate-800 break-words" {...props} />,
                                    h3: ({ node, ...props }) => <h3 className={`text-base sm:text-xl font-bold mb-3 sm:mb-4 mt-4 sm:mt-8 text-${accent}-700 flex items-center gap-2 break-words`} {...props} />,
                                    p: ({ node, ...props }) => <p className="mb-4 leading-relaxed text-sm sm:text-base break-words whitespace-pre-line" {...props} />,
                                    table: ({ node, ...props }) => (
                                    ...
                                    ul: ({ node, ...props }) => <ul className="list-none space-y-3 sm:space-y-5 my-4 sm:my-8 pl-0 max-w-full" {...props} />,
                                    ol: ({ node, ...props }) => <ol className="list-decimal space-y-3 sm:space-y-5 my-4 sm:my-8 pl-5 max-w-full" {...props} />,
                                    li: ({ node, ...props }) => (
                                    ...
                                    a: ({ node, ...props }) => (
                                        <a
                                            className={`text-${accent}-600 hover:text-${accent}-800 font-black underline underline-offset-4 decoration-2 decoration-${accent}-500/30 transition-all break-all`}
                                            target="_blank"
                                        ...
                                        <div className="my-6 sm:my-10 space-y-3 sm:space-y-4 max-w-full">
                                            <div className="relative group overflow-hidden rounded-xl sm:rounded-[2rem] border border-slate-100 shadow-md transition-all max-w-full">
                                                ...
                                                <div className="absolute top-2 left-2 sm:top-4 sm:left-4 px-2 sm:px-4 py-1 sm:py-2 bg-slate-900/80 backdrop-blur-md text-white text-[8px] sm:text-[9px] font-black uppercase tracking-widest rounded-lg sm:rounded-xl flex items-center gap-1 sm:gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <ImageIcon size={10} className={`text-${accent}-400`} />
                                                    Visual Reference
                                                </div>
                                            </div>
                                            {props.alt && (
                                                <p className="text-center text-[10px] sm:text-xs font-bold text-slate-400 italic px-4 sm:px-10 leading-relaxed uppercase tracking-wider break-words">
